 ...
 * ```
 */
export function defineApp(contract: AppContract): AppContract {
  return contract;
}
...
export function validateContract(raw: unknown): AppContract {
  if (typeof raw !== "object" || raw === null) {
    throw new Error("app.contract.ts must export a valid AppContract object");
  }
  const c = raw as Partial<AppContract>;
