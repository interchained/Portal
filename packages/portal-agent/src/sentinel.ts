...
import { AiAssistClient, type AiAssistConfig } from "./aiassist.js";
import type { AppContract } from "@interchained/portal-contract";
import { applyPatch, type Patch, type PatchStore } from "./patch.js";
...
Respond as JSON only — no markdown fences:
{
  ...
  "annotations": []
    ...
    try {
      ...
      return JSON.parse(cleaned) as SentinelReview;
    } catch {
      return {
        approved: false,
...
Respond as JSON only — no markdown fences:
{
  ...
  "summary": "one sentence describing what was corrected"
    ...
    try {
      const cleaned = raw.replace(/^```json\s*/i, "").replace(/\s*```$/, "").trim();
      return JSON.parse(cleaned) as SentinelFix;
    } catch {
  ...
  async reviewAndApply(opts: {
    ...
    const patchToApply = { ...opts.patch, proposed: finalContent, status: "approved" as const };
