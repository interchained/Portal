 ...
 */
function rerootUrl(url: string, publicOrigin: string | undefined): string {
  if (!publicOrigin || !url) return url;
  try {
...
function metaTags(route: BridgeRoute, options: RenderOptions): string {
  const seo = route.seo;
  ...
  if (tw.image) lines.push(`<meta name="twitter:image" content="${escapeAttr(tw.image)}">`);
...
function navHtml(menus: BridgeMenu[]): string {
  const primary =
    menus.find((menu) => menu.locations.length > 0) ?? menus[0];
  if (!primary || primary.items.length === 0) return "";
