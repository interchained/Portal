...
export { BridgeClient, FileSnapshotStore, MemorySnapshotStore, NedbSnapshotStore, siteKeyFromBaseUrl, WordPressBridgeSource, };
...
export { renderRouteHtml, renderSitemapXml } from "./render.js";
export { sanitizeWordPressHtml } from "./sanitize.js";
export { WP_CONTENT_CSS } from "./wp-content-css.js";
/** Explicit-config factory. */
export function wordpressPortalBridge(options) {
    if (!options.baseUrl || !/^https?:\/\//.test(options.baseUrl)) {
        throw new BridgeConfigError("wordpressPortalBridge: baseUrl must be an absolute http(s) URL (the canonical WordPress origin).");
    }
    if (!options.tmk || options.tmk.length < 24) {
        throw new BridgeConfigError("wordpressPortalBridge: tmk is missing or too short — copy PORTAL_TMK from WordPress Admin → Portal Bridge.");
    }
...
export function isBridgeConfigured(env = process.env) {
...
export function wordpressPortalBridgeFromEnv(env = process.env, overrides = {}) {
    const baseUrl = env["PORTAL_BRIDGE_BASE_URL"];
    const tmk = env["PORTAL_TMK"];
    if (!baseUrl || !tmk) {
        throw new BridgeConfigError("WordPress Portal Bridge requires PORTAL_BRIDGE_BASE_URL and PORTAL_TMK. " +
            "Generate both in WordPress Admin → Portal Bridge → Connect Portal Frontend.");
    }
    const mode = env["PORTAL_BRIDGE_MODE"] ?? undefined;
    if (mode && mode !== "live" && mode !== "snapshot-first") {
        throw new BridgeConfigError(`PORTAL_BRIDGE_MODE must be "live" or "snapshot-first", got "${mode}".`);
    }
