...
function makeSnapshot(routes, id = "snap_mock1") {
    return {
        ...
        const server = createServer((req, res) => {
            ...
            if (url.pathname.endsWith("/snapshot")) {
                ...
                if (route)
                    ...
                    respond(404, { code: "wpb_route_not_found", message: "No published route at that path." });
            }
...
test("wrong TMK is rejected by the (independent) verifier", async () => {
    ...
    await assert.rejects(() => source.ready(), (error) => error instanceof BridgeAuthError && error.status === 401);
});
test("env factory: fails clearly on missing or invalid env", () => {
    assert.throws(() => wordpressPortalBridgeFromEnv({}), BridgeConfigError);
    assert.throws(() => wordpressPortalBridgeFromEnv({
