...
test("iframes, forms, and style blocks are dropped", () => {
    const out = sanitizeWordPressHtml('<iframe src="https://evil.test"></iframe><form action="/x"><input></form><style>body{}</style><p>ok</p>');
...
test("WordPress/Gutenberg markup is preserved", () => {
    const html = '<figure class="wp-block-image size-full alignwide"><img src="/up/1.jpg" class="wp-image-42" alt=""><figcaption>Cap</figcaption></figure>' +
...
test("inert style attributes survive, url() styles are dropped", () => {
    const out = sanitizeWordPressHtml('<p style="text-align:center">a</p><div style="background:url(javascript:x)">b</div>');
