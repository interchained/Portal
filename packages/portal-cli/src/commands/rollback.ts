...
export async function rollbackCommand(opts: { id?: string } = {}): Promise<void> {
  banner();
  ...
  if (!targetId) {
    ...
    targetId = selected as string | undefined;
  }

  if (!targetId) { blank(); return; }
  ...
  if (!patch.original) {
    ...
    if (!confirm) { info("Cancelled."); blank(); return; }
