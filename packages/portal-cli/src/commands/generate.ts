...
function apiRouteTemplate(name: string): string {
  const fnName = name.replace(/-([a-z])/g, (_, c: string) => c.toUpperCase());
...
export async function generateCommand(
  ...
  if (sub === "api") {
    ...
    if (!opts.auto) {
      ...
      if (choice !== "yes") { info("Cancelled."); blank(); return; }
    }
  ...
  if (sub === "component") {
    ...
    try {
      ...
      spinner.stop();
      fail(`Generation failed: ${(err as Error).message}`);
      process.exit(1);
    }
    ...
    if (!opts.auto) {
      ...
      if (choice !== "yes") { info("Discarded."); blank(); return; }
    }
  ...
  let result: Awaited<ReturnType<typeof generateFromPrompt>>;
  try {
  ...
  if (sentinelApplied) {
    ...
    if (sentinelFix) {
      ...
      blank();
      sentinelFix.changes.forEach((c) =>
        console.log(`   ${icon.pass} ${pc.cyan(c)}`)
      );
    } else {
      header(`${icon.pass} Sentinel applied (clean): ${patch.file}`);
      if (sentinelReview) info(sentinelReview.summary);
    }
