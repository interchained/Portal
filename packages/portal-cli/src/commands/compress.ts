...
export async function compressCommand(dir = "dist"): Promise<void> {
  banner();
  ...
  const files = (await walk(target)).filter(
    (f) =>
      !f.endsWith(".br") &&
      !f.endsWith(".gz") &&
  ...
  for (const file of files) {
    ...
    if (raw.length < 256) { skipped++; continue; }
    ...
    if (wroteAny) {
      ...
      console.log(
        `   ${icon.pass} ${rel.padEnd(40).slice(0, 40)} ` +
  ...
  console.log(
    pc.dim("   raw    ") + pc.white(fmtBytes(rawTotal).padStart(10))
  );
  console.log(
    pc.dim("   gzip   ") + pc.green(fmtBytes(gzTotal).padStart(10)) + pc.dim(`   −${gzPct}%`)
  );
  console.log(
    pc.dim("   brotli ") + pc.green(fmtBytes(brTotal).padStart(10)) + pc.dim(`   −${brPct}%`) +
