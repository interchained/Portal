...
export async function doctorCommand(opts: { fix?: boolean } = {}): Promise<void> {
  ...
  if (hasRoutes) {
    for (const file of pageFiles) {
      ...
      if (base.endsWith("/index") || base === "index") continue;
  ...
  if (contractPath) {
    ...
    for (const [name, filePath] of Object.entries(contract.data ?? {})) {
      const fileExists = await exists(join(root, filePath as string));
      checks.push({
    ...
    if (check.status === "ok") {
      ...
      if (opts.fix && check.fix) {
        try {
          ...
          console.log(pc.red(`   ✗ Fix failed: ${(err as Error).message}`));
        }
      } else if (check.fix) {
