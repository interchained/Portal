...
import { validateContract, type AppContract } from "@interchained/portal-contract";
...
export async function findContractPath(root: string): Promise<string | null> {
  for (const candidate of CANDIDATE_PATHS) {
    const full = join(root, candidate);
    try { await access(full); return full; } catch { /* next */ }
  }
  return null;
}

export async function loadContract(root = process.cwd()): Promise<AppContract> {
  const contractPath = await findContractPath(root);
  if (!contractPath) return { name: "Portal App", goals: [] };
