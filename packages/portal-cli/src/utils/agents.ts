...
export async function agentsDirExists(agentsDir: string): Promise<boolean> {
  try { await access(agentsDir); return true; } catch { return false; }
}
 ...
 */
export function matchAgentFile(files: string[], name: string): string | null {
  return (
    files.find((f) => f === `${name}.agent.ts` || f === `${name}.agent.js`) ??
...
function evalAgentObject(src: string): AgentDefinition {
  const cleaned = src
